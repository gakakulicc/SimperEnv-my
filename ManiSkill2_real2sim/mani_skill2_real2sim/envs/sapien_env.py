...
class BaseEnv(gym.Env):
    ...
    def __init__(
        ...
        # Impulse parameters
        self.impulse_force = np.array([1, 0, 0])  # 定义施加的力
        self.impulse_duration = 1  # 定义施加力的持续时间
        self.impulse_step = 10  # 定义在第 10 步施加力
    ...
    def apply_force_to_robot_link(self):
        # 定义施加力的条件
        ...
        # for link in links:
        #     if isinstance(link, sapien.ActorDynamicBase):
        #         print(f"Link {link.name} is dynamic.")
        #     else:
        #         print(f"Link {link.name} is not dynamic.")

        # 获取链接的中心点
        link_pose = link.get_pose()
        link_center = link_pose.p
        print(f"Link {link.name} center: {link_center}")

        # 定义施加的力的大小和方向
        force = np.array([0, 0, 1000])  # 在 z 轴方向施加 100 的力

        # 定义射线的方向（例如，沿着z轴正方向）
        ray_direction = np.array([0, 0, 1])

        # 发射射线
        ray_origin = link_center  # 射线起点为目标点
        ray = sapien.Ray(ray_origin, ray_direction)

        # 检测射线与场景的交点
        hits = self._scene.cast_ray(ray_origin, ray_direction, 10.0)  # 最大检测距离为10.0
        ...
        initial_position = link.get_pose().p.copy()  # 获取初始位置

        # 调用 add_force_at_point 方法施加力
        link.add_force_at_point(force, point)

        print(f"Force {force} applied at point {point} on link {link.name}.")

        final_position = link.get_pose().p.copy()  # 获取最终位置

        # 计算位置变化
        displacement = final_position - initial_position
        print(f"Link {link.name} moved from {initial_position} to {final_position}.")
        print(f"Displacement: {displacement}")

        # 判断是否移动
        if np.linalg.norm(displacement) > 1e-4:  # 设置一个阈值来判断是否移动
            print("The robot link has moved due to the applied force.")
        else:
            print("The robot link has not moved significantly. Check the force or simulation settings.")
    ...
    def _before_control_step(self):
        # # 在这里给机器人的某个链接施加一个短暂的力
        # if self.impulse_step <= self._elapsed_steps < self.impulse_step + int(self.impulse_duration * self.control_freq):
        #     print("action/action/action")
        #     self.apply_force_to_robot_link()
